module ApplicationHelper
    ...
    if @is_block_section
      ...
      if %w[tezos filecoin cosmoshub].include? @network['network']
        button_prev = link_to('&larr;'.html_safe,
                              url_for(controller: :height, height: prev_height, action: params[:action]), class: 'btn btn-outline-secondary btn-sm block-buttons')
        button_next = link_to('&rarr;'.html_safe,
                              url_for(controller: :height, height: next_height, action: params[:action]), class: 'btn btn-outline-secondary btn-sm block-buttons')

      elsif ['solana'].include? @network['network']
        prev_height = (@block_id.to_i - 1).to_s
        next_height = (@block_id.to_i + 1).to_s
        button_prev = link_to('&larr;'.html_safe,
                              url_for(controller: :block, block_id: prev_height, action: params[:action]), class: 'btn btn-outline-secondary btn-sm block-buttons')
        button_next = link_to('&rarr;'.html_safe,
                              url_for(controller: :block, block_id: next_height, action: params[:action]), class: 'btn btn-outline-secondary btn-sm block-buttons')
        addr = @block_id
      else
        button_prev = link_to('&larr;'.html_safe,
                              url_for(controller: :block, block: prev_height, action: params[:action]), class: 'btn btn-outline-secondary btn-sm block-buttons')
        button_next = link_to('&rarr;'.html_safe,
                              url_for(controller: :block, block: next_height, action: params[:action]), class: 'btn btn-outline-secondary btn-sm block-buttons')
      end
    end
  ...
  def tab_ads(html_class = 'nav-item nav-item-ad')
    return unless (ads = current_ad(:tab, :ads))

    ads.filter_map do |ad|
      ...
      tag.li(class: html_class) do
        link_to ad_url, class: 'nav-link nav-link-ad',
  ...
  def tab_ad(html_class = 'nav-item nav-item-ad')
    return unless (ad = current_ad(:tab, :ad))
