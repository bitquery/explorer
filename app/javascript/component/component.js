import { isNotEmptyArray, isNotEmptyObject } from "./helper";

const mapQueryAndVariablesStore = new Map();

const subscribeWidget = async (subscribeWith, widgetInstance, onerror) => {
	const currentUrl = subscribeWith.endpoint_url.replace(/^http/, 'ws');
	const client = createClient({ url: currentUrl });
	const payload = { query: subscribeWith.query, variables: subscribeWith.variables };
