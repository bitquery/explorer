export default class GraphsComponent {
  ...
  async onData(data, sub) {
      ...
      for (const rowData of array) {
              ...
              if (column.name === 'Sender' || column.name === 'Receiver') {
                  ...
                  if (!addresses.has(cellData)) {
                      ...
                      if (column.name === 'Sender') {
                          nodes.push({ id: cellData, label: this.shortenText(cellData), url: `/${this.variables.networkForURL}/address/${dataObject['Sender']}/nft_address` });
                      }
                      if (column.name === 'Receiver') {
                          nodes.push({ id: cellData, label: this.shortenText(cellData), url: `/${this.variables.networkForURL}/address/${dataObject['Receiver']}/nft_address` });
                      }
                  }
              }
              if (column.name === 'Time') {
          ...
          edges.push({ from: dataObject['Sender'], to: dataObject['Receiver'], label: dataObject['Time'], url: `/${this.variables.networkForURL}/tx/${dataObject['TX Hash']}` });
      }
