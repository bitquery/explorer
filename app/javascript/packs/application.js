...
function setupWalletConnection() {
  ...
  const setupConnectionButton = () => {
    if (connectionList.length) {
      ...
      const woIcon = document.createElementNS(
        'http://www.w3.org/2000/svg',
        'svg'
      );
      woIcon.classList.add('wallet-icon');
      const woUseElem = document.createElementNS(
        'http://www.w3.org/2000/svg',
        'use'
      );
      woUseElem.setAttributeNS(
      ...
      const walletConnectionButton =
        document.querySelector('#wallet-connection');
      ...
      addNewConnectionButton.classList.add(
        'btn',
        'btn-primary',
        'add-connection',
...
global.createLayout = function (
  ...
  new_layout_element_container.style.transform = `translate(${
    layout_item.x * unit.width + 10
  }px, ${layout_item.y * unit.height + 10}px)`;
  new_layout_element_container.style.width = `${
    layout_item.w * unit.width - 10
  }px`;
  new_layout_element_container.style.height = `${
    layout_item.h * unit.height - 10
  }px`;
  ...
  $.get(`/proxy_dbcode/${dashboard_url}`, function (data) {
    ...
    data.widgets.forEach((widget) => {
      ...
      if (widget.widget_id === 'block.content') {
        ...
        widgetRenderer[widget.widget_id](
          ds,
