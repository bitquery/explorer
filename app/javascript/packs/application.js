...
global.createWidget = async function (widgetType, container_id, argsReplace, transferURL) {
  const query = {
    transfersETH: "query transfers($network: evm_network!) {\n  EVM(network: $network) {\n    Transfers(\n      where: {Transfer: {Currency: {Native: true}}}\n      limit: {count: 100}\n      ) {\n      Block {\n        Number\n        Time\n      }\n      Transfer {\n        Currency {\n          Symbol\n        }\n        Receiver\n        Sender\n        Amount\n      }\n      Transaction{\n        Hash\n      }\n    }\n  }\n}\n",
  ...
  const variables = {}
  const dataFunction = {
    transfersETH: data => data.EVM.Transfers.filter(el => el.Transfer.Sender !== '0x' && el.Transfer.Receiver !== '0x'),
    ...
    token_dex_trades: {
      ...
      columns: [
        ...
        },
        {
          ...
          formatterParams: {
            url: (cell) =>
              `${window.location.origin}/${argsReplace.network}/token/${cell.getData().Trade.Sell.Currency.SmartContract
              }`,
          },
        },
        {
