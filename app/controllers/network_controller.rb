class NetworkController < ApplicationController
  ...
  def breadcrumbs
    @breadcrumbs = [
        ...
        (params[:token1] ? {name: "#{t("tabs.#{controller_name}.show.name")}: #{params[:token1].truncate(15)}", url: "#{locale_path_prefix}#{@network[:network]}/token/#{params[:token1]}"} : nil),
        (params[:token2] ? {name: "#{t("tabs.#{controller_name}.show.name")}: #{params[:token2].truncate(15)}", url: "#{locale_path_prefix}#{@network[:network]}/token/#{params[:token2]}"} : nil),
